...
## What it does
The visualizer presents rankings, summary totals, individual task scores, score distributions with adjustable bucket size, highlighted competitors, percentile box plots, person-to-person task comparisons, correlation plots, a correlation matrix, regional averages, task difficulty, theory/practical splits, and a sortable detailed table. The control rail is scoped to the active tab, and the KPI/insight cards are only shown on Overview.
